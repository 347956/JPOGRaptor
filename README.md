...
This repository contains the full source code for the JPOGRaptor for Lethal Company, including the Unity project which can be used to build its asset bundle. The guide part of this project can be found at https://lethal.wiki/dev/apis/lethallib/custom-enemies/overview.
...
- When renaming [JPOGRaptorAI.cs](/Plugin/src/JPOGRaptorAI.cs) and its class, make sure to compile your mod DLL and place it in `./UnityProject/Assets/Plugins` and add your new AI class as a component to your enemy prefab. If the class doesn't appear as an option, make sure your mod DLL doesn't have any errors in the Unity editor console. If your mod depends on other mods, also add their DLL files to the `Plugins` folder.
...
> [!TIP]  
> You can use `Ctrl` + `Shift`+ `F` to search every file in both Visual Studio and Visual Studio Code. This can for example help you find every instance of `JPOGRaptor` inside files in the whole project. Do note however that this does not apply to filenames.  
...
[XuuXiao](https://github.com/XuuXiao/) - porting LC-JPOGRaptor for LC v50  
